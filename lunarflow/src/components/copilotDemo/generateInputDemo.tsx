'use client'
import { autoCreateWorkflowAction, autoCreateWorkflowDemoAction } from '@/app/actions/copilot';
...
const GenerateInputDemo: React.FC<GenerateInputProps> = ({ }) => {
  ...
  const autoCreateWorkflowRequest = (intent: string) => {
    setIsLoading(true)
    if (userId) {
      autoCreateWorkflowDemoAction(intent, userId)
        .then(({ id }) => {
...
export default GenerateInputDemo;
