...
from .jupyter_server_manager import JupyterServerConfigModel, JupyterServerManager
...
class NotebookController:
    ...
    async def open(self, workflow: WorkflowModel, user_id: str, host: str, port: int):
        nb_path = self._persistence_layer.get_user_workflow_notebook_path(workflow_id=workflow.id, user_id=user_id)

        nb_exists = await self._persistence_layer.file_exists(f"{nb_path}index.ipynb")
        if not nb_exists:
            await self.save(workflow, user_id)

        jupyterConfig = JupyterServerConfigModel(
            host=host,
            port=port,
            ...
            kernel_display_name=workflow.name,
        )
